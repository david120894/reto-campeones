...
import { SeminarAttendance } from '../../../../../core/models/seminar-attendance'
...
@Component({
  ...
  styleUrl: './seminar-attendance.component.scss',
})
export class SeminarAttendanceComponent implements OnInit, OnDestroy {
  private subscriptions = new Subscription()
  loading = true
  listToExcel: any[] = []
  totalInscriptions = 0
  listParticipantsSeminar: SeminarAttendance[] = []
  listParticipantsAux: SeminarAttendance[] = []
  paginatedReservations: SeminarAttendance[] = []
  currentPage = 1
  itemsPerPage = 10
  totalPages = 1
  ...
  searchForm: FormGroup = new FormGroup({
    ...
    selectedDate: new FormControl(new Date().toISOString().split('T')[0]),
  })
  ...
  getParticipantsSeminar() {
    ...
    const sub = this.participantsService.getParticipantsAttendanceSeminar(selectedDate).subscribe({
      next: (response) => {
